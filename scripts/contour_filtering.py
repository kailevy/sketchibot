    ...
    def plot_points(self):
        ...
        while not rospy.is_shutdown():
            ...
            for path in self.strokes:
                for i in range(len(path)):
                    if 0<i<len(path):
                        cv2.circle(self.im,(int(path[i][0]*self.screenscale),int(path[i][1]*self.screenscale)),1,(0,0,0),2)                    

    def get_strokes(self):
        return self.strokes()

if __name__ == '__main__':
    detector = EdgeDetector(image_path="../images/cow.png")
    detector.reconstruct_contours()
    detector.sort_contours()
    contours = detector.get_contours()
    size = detector.get_size()
    print size
    drawing = ContourFiltering(strokes = contours,imsize=size,pagesize=(2,2))
    drawing.run_filter()